...
def find_repo_root(start: Path) -> Path:
    ...
    while True:
        if (p / "pidControllers").is_dir():
            return p
        if p.parent == p:
            raise RuntimeError("Could not locate repo root containing 'pidControllers'")
        p = p.parent
...
def test_cli_lists_and_compute(tmp_path: Path):
    # Ensure pidControllers is a package (for imports in subprocess)
    pkg_init = REPO_ROOT / "pidControllers" / "__init__.py"
    ...
    proc1 = subprocess.run(
        [sys.executable, "-m", "pidControllers.tuningTool.cli", "--file", str(json_path), "--list", "methods"],
    ...
    proc2 = subprocess.run(
        [sys.executable, "-m", "pidControllers.tuningTool.cli",
