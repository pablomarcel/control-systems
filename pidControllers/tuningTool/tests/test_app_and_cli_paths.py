...
PKG_DIR = REPO_ROOT / "pidControllers" / "tuningTool"
...
def test_cli_list_methods_with_in_prefix(tmp_path):
    ...
    cmd = [sys.executable, "-m", "pidControllers.tuningTool.cli", "--file", "in/tuning_rules.json", "--list", "methods"]
...
def test_cli_unknowns_and_missing(tmp_path):
    rules = write_rules(tmp_path)
    # Unknown method
    proc = subprocess.run([sys.executable, "-m", "pidControllers.tuningTool.cli", "--file", "tuning_rules.json", "--method", "BAD", "--controller", "PID"],
                          cwd=str(REPO_ROOT), capture_output=True, text=True)
    assert proc.returncode == 2 and "Unknown method" in proc.stdout
    # Missing inputs
    proc2 = subprocess.run([sys.executable, "-m", "pidControllers.tuningTool.cli", "--file", "tuning_rules.json", "--method", "ZN_step", "--controller", "PID"],
                           cwd=str(REPO_ROOT), capture_output=True, text=True)
