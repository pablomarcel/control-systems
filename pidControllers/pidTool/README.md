...
```bash
python -m pidControllers.pidTool.cli run   --plant-form poly   --num-poly "1.2"   --den-poly "0.36*s**3 + 1.86*s**2 + 2.5*s + 1"   --structure pid_dz   --K-range 6 20 --K-n 40   --a-range 0.45 1.8 --a-n 45   --tfinal 25 --dt 0.002   --settle-tol 0.05   --objective mp   --backend none --export-json --save-prefix dz_os_scout_demo
```
...
```bash
python -m pidControllers.pidTool.cli run \
  --plant-form poly \
  --num-poly "1.2" \
