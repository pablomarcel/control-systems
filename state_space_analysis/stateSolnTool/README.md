...
CLI entry point:
```bash
python -m state_space_analysis.stateSolnTool.cli [OPTIONS]
```
...
## 0) Quick help
```bash
python -m state_space_analysis.stateSolnTool.cli --help
```
...
```bash
python -m state_space_analysis.stateSolnTool.cli \
  --example ogata_9_1 \
...
```bash
python -m state_space_analysis.stateSolnTool.cli --example ogata_9_1 --u 1 --verify
```

### B) Failure mode (missing `--den`): should print an error & nonzero exit
```bash
python -m state_space_analysis.stateSolnTool.cli --num "1,2,3"
```
