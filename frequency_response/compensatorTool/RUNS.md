...
python cli.py --ogata_7_28 \
  ...
  --export_csv_prefix "out/og728" \
  --export_json "out/og728_design.json" \
  --save "out/og728_{kind}.png" \
  --no_show
```
...
python cli.py --mode lead \
  ...
  --export_json "out/lead_kv.json" \
  --export_csv_prefix "out/lead_kv" \
  --save "out/lead_kv_{kind}.png"
```
...
python cli.py --mode lead \
  ...
  --save "out/lead_2stage_{kind}.png"
```
...
python cli.py --mode lead \
  ...
  --save "out/lead_manual_{kind}.png"
```
...
python cli.py --mode lead \
  ...
  --save "out/lead_nichols_{kind}.png"
```
...
python cli.py --mode lead \
  ...
  --nichols_Mdb -12 -9 -6 -3 0 3 6 9 12 \
  --nichols_Ndeg -30 -60 -90 -120 -150 -180 \
  --save "out/lead_nichols_custom_{kind}.png"
```
...
python cli.py --mode lead \
  ...
  --save "out/lead_full_{kind}.html" \
  --save_img "out/lead_full_{kind}.png" \
  --export_json "out/lead_full.json" \
  --export_csv_prefix "out/lead_full"
```
...
```bash
python cli.py --mode lag --help
```

## 1) Minimal auto design (MPL, Bode only)

```bash
python cli.py --mode lag \
  --num 4 --den "1, 2, 0" \
  --lag_pm_target 45 --lag_pm_add 8 \
  --backend mpl --plots bode --no_show \
  --save "out/lag_min_{kind}.png"
```
...
python cli.py --mode lag \
  ...
  --lag_pm_target 45 --lag_pm_add 8 \
  --backend mpl --plots bode,nyquist,nichols,step,ramp --no_show \
  --export_json "out/lag_kv.json" \
  --export_csv_prefix "out/lag_kv" \
  --save "out/lag_kv_{kind}.png"
```
...
python cli.py --mode lag \
  ...
  --lag_pm_target 45 --lag_pm_add 8 \
  --backend mpl --plots nichols --nichols_templates --no_show \
  --save "out/lag_nichols_{kind}.png"
```
...
python cli.py --mode lag \
  ...
  --nichols_Mdb -12 -9 -6 -3 0 3 6 9 12 \
  --nichols_Ndeg -30 -60 -90 -120 -150 -180 \
  --save "out/lag_nichols_custom_{kind}.png"
```
...
python cli.py --mode lag \
  ...
  --lag_pm_target 45 --lag_pm_add 8 \
  --backend plotly --plots bode,nyquist,nichols --no_show \
  --save "out/lag_full_{kind}.html" \
  --save_img "out/lag_full_{kind}.png" \
  --export_json "out/lag_full.json" \
  --export_csv_prefix "out/lag_full"
```
...
python cli.py --mode lag \
  ...
  --ogata_axes \
  --save "out/lag_og_axes_{kind}.png"
```
...
python cli.py --mode lag \
  ...
  --save "out/lag_nichols_cl_{kind}.html"
```
