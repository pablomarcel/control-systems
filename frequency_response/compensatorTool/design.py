...
from .utils import db, parse_list_floats, parse_params, tf_from_expr
...
class LagLeadDesigner:
    ...
    def preset_ogata_7_28(G_base: ct.lti) -> Tuple[LagLeadDesignNumbers, ct.lti]:
        # Ogata Example 7-28 baseline gain factor
        G1 = 20 * ct.tf(G_base)
        ...
        # Lag (wz=0.15, wp=0.015)
        wz_lag, wp_lag = 0.15, 0.015
        beta = wz_lag / wp_lag
        T_lag = 1.0 / wz_lag

        return (
            LagLeadDesignNumbers(
                alpha, beta, T_lead, T_lag, wz_lead, wp_lead, wz_lag, wp_lag, 1.0, float("nan")
            ),
            G1,
        )
    ...
    def auto_design(
        ...
        # Choose wc based on target phase line
        target_phase = -180.0 + pm_target + pm_allow_extra
        idx = int(np.argmin(np.abs(ph_deg - target_phase)))
        wc = float(ww[idx] if wc_hint is None else wc_hint)

        # How much phase to add
        _, pm, _, _ = get_margins(G1)
        needed = max(0.0, pm_target - pm + pm_allow_extra)

        # Lead
        alpha, T_lead, wz_lead, wp_lead = LagLeadDesigner._lead_from_phase_add(
            needed, wc, r=r_lead
        )

        # Lag to recover low-freq gain lost by lead
        Glead = ct.tf([T_lead, 1.0], [alpha * T_lead, 1.0])
        magL, _, _ = frequency_response_arrays(Glead * G1, np.array([wc]))
        beta = float(max(1.001, magL[0]))  # ensure >1

        wz_lag = wc / max(1.001, r_lag)
        T_lag = 1.0 / wz_lag
        wp_lag = wz_lag / beta

        return LagLeadDesignNumbers(
            alpha, beta, T_lead, T_lag, wz_lead, wp_lead, wz_lag, wp_lag, 1.0, wc
        )
    ...
    def run(self, spec: LagLeadDesignSpec) -> LagLeadDesignResult:
        ...
        if spec.design.ogata_7_28:
            ...
            # If Kv is specified, rescale the plant to meet Kv
            ...
            if all(v is not None for v in [d.wz_lead, d.wp_lead, d.wz_lag, d.wp_lag]):
                # Manual placement given, infer alpha/beta if needed
                ...
                nums = LagLeadDesignNumbers(
                    alpha=float(alpha),
                    beta=float(beta),
                    T_lead=1.0 / float(d.wz_lead),
                    T_lag=1.0 / float(d.wz_lag),
                    wz_lead=float(d.wz_lead),
                    wp_lead=float(d.wp_lead),
                    wz_lag=float(d.wz_lag),
                    wp_lag=float(d.wp_lag),
                    Kc=float(d.Kc),
                    wc=float("nan"),
                )
            else:
                nums = self.auto_design(
                    G1,
                    d.Kv if d.Kv is not None else 0.0,
                    pm_target=float(d.pm_target if d.pm_target is not None else 50.0),
                    pm_allow_extra=float(d.pm_allow),
                    wc_hint=d.wc_hint,
                    r_lead=float(d.r_lead),
                    r_lag=float(d.r_lag),
                )

        # Build compensator and closed-loop systems
        ...
        pack = {
            ...
            "lag_lead": {
                "alpha": nums.alpha,
                "gamma": 1 / nums.alpha if nums.alpha > 0 else float("inf"),
                "T2": nums.T_lead,
                "wz_lead": nums.wz_lead,
                "wp_lead": nums.wp_lead,
                "beta": nums.beta,
                "T1": nums.T_lag,
                "wz_lag": nums.wz_lag,
                "wp_lag": nums.wp_lag,
            ...
            "notes": {
                "baseline_used": "gain_adjusted"
                if (spec.design.ogata_7_28 or spec.design.Kv is not None)
                else "original"
            },
        }
        ...
        files += io_mod.render_plots(
            ...
            nichols_Mdb=parse_list_floats(plot.nichols_Mdb)
            or (
                [-12, -9, -6, -3, 0, 0.9, 3, 6, 9, 12]
                if plot.nichols_templates
                else None
            ),
            nichols_Ndeg=parse_list_floats(plot.nichols_Ndeg)
            or ([-30, -60, -90, -120, -150, -180] if plot.nichols_templates else None),
            ...
            show_unstable=plot.show_unstable,  # <-- pass through (NEW)
        )
    ...
    def _build_plant(self, plant: PlantSpec, params: dict) -> ct.lti:
        ...
        if plant.A and plant.B and plant.C:
            A = np.array(
                [
                    [float(self._eval(x, params)) for x in r.strip().split(",")]
                    for r in plant.A.split(";")
                ]
            )
            B = np.array(
                [
                    [float(self._eval(x, params)) for x in r.strip().split(",")]
                    for r in plant.B.split(";")
                ]
            )
            C = np.array(
                [
                    [float(self._eval(x, params)) for x in r.strip().split(",")]
                    for r in plant.C.split(";")
                ]
            )
            D = (
                np.array([[0.0]])
        ...
        # TF numerator/denominator arrays
