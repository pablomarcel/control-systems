...
class PlotService:
    ...
    def _coerce_req(self, req_like: Any) -> PlotRequest:
        """Turn any req-like object into a fully-populated PlotRequest."""
        ...
        save_html_base = (
            self._get(req_like, "save_html_base", None)
            or self._get(req_like, "save_html_dir", None)
            or self._get(req_like, "save_html", None)
        )
        ...
        if isinstance(req.save_png_dir, str) and req.save_png_dir:
            Path(req.save_png_dir).mkdir(parents=True, exist_ok=True)
        if isinstance(req.save_html_base, str) and req.save_html_base:
            Path(req.save_html_base).parent.mkdir(parents=True, exist_ok=True)
        if isinstance(req.save_json_path, str) and req.save_json_path:
            Path(req.save_json_path).parent.mkdir(parents=True, exist_ok=True)
    ...
    # ------------------- per-channel work -------------------
    def run_channel(
        self, idx: int, total: int, args_any: Any, L: ct.TransferFunction, w: np.ndarray, name: str, req: PlotRequest
    ):
        ...
        png = (
            lambda stub: None
            if not req.save_png_dir
            else os.path.join(req.save_png_dir, f"{stub}{'' if total == 1 else f'_c{idx+1}'}.png")
        )
            ...
            try:
                if req.plotly:
                    ...
                    bode_p(
                        ...
                        w,
                        margins,
                        req.title if total == 1 else f"{req.title} — {name}",
                        False,
                        False,
                        html("bode"),
                        req.wmarks,
        ...
        if req.nyquist:
            try:
                if req.plotly:
                    ...
                    nyq_p(
                        L, w, req.title if total == 1 else f"{req.title} — {name}", req.nyq_markers, req.nyq_samples, html("nyquist")
                    )
                else:
            ...
            try:
                if req.plotly:
                    ...
                    nich_p(
                        ...
                        req.title if total == 1 else f"{req.title} — {name}",
                        req.nichols_grid,
                        req.nichols_closedloop,
                        html("nichols"),
                        req.nichols_range,
                        m_levels,
                        n_levels,
                        (not req.nichols_no_grid_labels),
                    )
                else:
