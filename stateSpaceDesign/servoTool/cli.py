...
Works in two modes:
  1) From project root (preferred):  python -m stateSpaceDesign.servoTool.cli --help
...
Note:
  Running `python -m stateSpaceDesign.servoTool.cli` from *inside* this folder
  will fail (Python cannot locate the top-level package from a subdir). Use
  `python cli.py` when you `cd stateSpaceDesign/servoTool/`.
"""
...
if __package__ in (None, ""):
    ...
    from stateSpaceDesign.servoTool.apis import RunRequest, run
else:
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog="stateSpaceDesign.servoTool",
