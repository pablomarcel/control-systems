# transient_analysis/hurwitzTool/tests/test_cli_smoke.py
...
def _find_repo_root() -> Path:
    ...
    for parent in here.parents:
        if (parent / "pyproject.toml").exists() or (parent / "pytest.ini").exists():
            return parent
    # Fallback: .../modernControl (tests are at .../modernControl/transient_analysis/hurwitzTool/tests)
...
def run_cmd(args: list[str]) -> tuple[int, str]:
    ...
    env = dict(os.environ)
    # Ensure the repo root is importable so `-m transient_analysis.hurwitzTool.cli` resolves.
    env["PYTHONPATH"] = os.pathsep.join([str(repo_root), env.get("PYTHONPATH", "")]).rstrip(os.pathsep)

    p = subprocess.Popen(
        [sys.executable, "-m", "transient_analysis.hurwitzTool.cli", *args],
        cwd=repo_root,
        env=env,
