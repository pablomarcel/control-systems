...
```bash
cd transient_analysis/responseTool
```
...
```bash
pytest transient_analysis/responseTool/tests \
  --override-ini addopts= \
  --cov \
  --cov-config=transient_analysis/responseTool/.coveragerc \
...
```bash
python cli.py --root transient_analysis/responseTool/out second-order --wn 5 --zeta 0.4 --plot
```
...
```bash
python cli.py second-order-overlays --root transient_analysis/responseTool/out/out --wn 5 --zetas "0,0.3,0.6,1.0" --tfinal 8 --dt 0.01 --save-prefix "overlay_run" --plot
```

```bash
python cli.py second-order-mesh --root transient_analysis/responseTool/out/out --wn 3 --zeta-min 0 --zeta-max 1 --zeta-steps 61 --tfinal 10 --dt 0.01 --save-prefix "mesh_run" --plot
```
