# modernControl/rootLocus/compensatorTool/tests/test_cli_smoke.py
...
# Tests live at modernControl/rootLocus/compensatorTool/tests
# We want PROJECT_ROOT = modernControl (one level above 'rootLocus')
THIS_FILE = Path(__file__).resolve()
ROOTLocus_DIR = THIS_FILE.parents[2]         # .../modernControl/rootLocus
...
def run_cli(args: List[str]) -> Tuple[int, str, List[str], dict]:
    ...
    # Putting rootLocus itself on the path prevents importing 'rootLocus' as a package.
    ...
    argv = [sys.executable, "-m", "rootLocus.compensatorTool.cli", *args]
    p = subprocess.Popen(
        argv,
...
def test_cli_help_smoke():
    """Smoke: `python -m rootLocus.compensatorTool.cli --help` returns 0 and shows banner."""
