...
class RootLocusApp:
    ...
    def run_batch(self, batch: RootLocusBatchSpec) -> str:
        ...
        for idx, case in enumerate(batch.cases, 1):
            cfg = {**batch.defaults, **case}
            req = RootLocusRequest(**cfg)
            self.log.info(f"--- Case {idx}/{len(batch.cases)}: {req.title} ---")
            outs = OutputSpec(out_dir=batch.outdir,
                              html=cfg.get("html") or f"{safe_title_to_filename(req.title)}.html",
                              png=cfg.get("png"),
                              csv=cfg.get("csv"))
            try:
                summaries.append(self.run(req, outs))
            except Exception as e:
                self.log.error(f"Case failed: {e}")
        lines = ["<html><head><meta charset='utf-8'><title>Root–Locus Report</title></head><body>",
        ...
        rpt = os.path.join(batch.outdir, batch.report)
        with open(rpt, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
        self.log.info(f"[saved] report -> {rpt}")
        return rpt
