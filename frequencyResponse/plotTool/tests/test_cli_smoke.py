...
def run_cmd(args: list[str]) -> tuple[int, str]:
    ...
    for p in [here] + list(here.parents):
        if (p / "frequencyResponse" / "plotTool" / "cli.py").exists():
            repo_root = p
            break
    assert repo_root, "Could not locate repo root containing frequencyResponse/plotTool/cli.py"
    cli_mod = repo_root / "frequencyResponse" / "plotTool" / "cli.py"
    ...
    cmd = [python, "-m", "frequencyResponse.plotTool.cli", *args]
