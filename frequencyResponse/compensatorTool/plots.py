...
from .core import frequency_response_arrays, get_margins, eval_L, is_stable
from .utils import db

# NOTE: Do NOT force a non-interactive backend here; let the environment decide.
# This ensures figures can pop open when running locally with Matplotlib.
import matplotlib.pyplot as plt
from matplotlib.ticker import MultipleLocator, LogLocator, NullFormatter


def _wrap_nichols(ph_rad: np.ndarray) -> np.ndarray:
    deg = np.rad2deg(ph_rad)
    deg = (deg + 360.0) % 360.0
    return deg - 360.0


def get_margins_safe(G: ct.lti) -> Tuple[float, float, float | None, float | None]:
...
def plot_bode_mpl(series, w, title: str = "Bode", wgm=None, wpm=None):
    ...
    _add_log_minor_grid(axm)
    _add_log_minor_grid(axp)
    axm.yaxis.set_major_locator(MultipleLocator(10))
    axm.yaxis.set_minor_locator(MultipleLocator(2))
...
def plot_nyquist_mpl(
    series,
    w,
    title: str = "Nyquist",
    ogata_axes: bool = False,
    Mlist: Optional[List[float]] = None,
    freq_marks: Optional[List[float]] = None,
):
    fig = _new_fig(figsize=(6.8, 6.8))
...
def plot_nichols_mpl(series, w, title: str = "Nichols", templates=None):
    fig = _new_fig(figsize=(10.8, 6.0))
    ax = fig.add_subplot(111)
    for label, G in series:
        mag, ph, _ = frequency_response_arrays(G, w)
    ...
    if templates:
        ...
        for phs, gdbs, name in M_lines:
            ax.plot(phs, gdbs, "--", lw=0.9, color="0.6")
...
def _coerce_forced_response(res):
    """
    Normalize python-control return type across versions:
    - Older versions: tuple (T, y) or (T, y, x)
    - Newer versions: TimeResponseData with .time/.outputs (or .t/.y)
    Returns (T, y).
    """
    # Tuple-like
...
def plot_step_mpl(
    ...
    ref_w = (
        wpm_c
        if np.isfinite(wpm_c) and wpm_c > 0
        else (wpm_u if np.isfinite(wpm_u) and wpm_u > 0 else 1.0)
    )
    ...
    # Only show baseline if stable or explicitly requested
    show_base = show_unstable or is_stable(Gcl_base)
    if show_base:
        T1, y1 = ct.step_response(Gcl_base, T=t)

    fig = _new_fig(figsize=(11.5, 3.8))
    ax = fig.add_subplot(111)
    if show_base:
...
def plot_ramp_mpl(
    ...
    # Robust unpack for python-control >=0.9 and older
    T2, y2 = _coerce_forced_response(ct.forced_response(Gcl_comp, T=t, U=u))

    show_base = show_unstable or is_stable(Gcl_base)
    if show_base:
        T1, y1 = _coerce_forced_response(ct.forced_response(Gcl_base, T=t, U=u))

    fig = _new_fig(figsize=(10.5, 6.0))
    ax = fig.add_subplot(111)
...
def bode_plotly(series, w, title: str = "Bode", wgm=None, wpm=None):
    if not _HAVE_PLOTLY:
    ...
    for label, G in series:
        mag, ph, _ = frequency_response_arrays(G, w)
        fig.add_trace(
            go.Scatter(x=w, y=db(mag), name=label, mode="lines"), row=1, col=1
        )
        fig.add_trace(
            go.Scatter(x=w, y=np.rad2deg(ph), name=label, mode="lines", showlegend=False),
            row=2,
            col=1,
        )
...
def nyquist_plotly(
    ...
    if freq_marks:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=pts.imag,
                mode="markers+text",
                text=[f"{w:g}" for w in freq_marks],
    ...
    fig.add_trace(
        go.Scatter(x=[-1], y=[0], mode="markers", marker=dict(color="black", size=8), name="-1+j0")
    )
    fig.update_layout(
        title=title,
        xaxis_title="Re{G(jw)}",
    ...
    if ogata_axes:
        ...
        fig.update_yaxes(range=[-8, 2])
    return fig


def nichols_plotly(series, w, title: str = "Nichols", templates=None):
    if not _HAVE_PLOTLY:
        raise RuntimeError("Plotly not available")
    fig = go.Figure()
    for label, G in series:
    ...
    fig.update_xaxes(range=[-360, 0])
    fig.update_yaxes(range=[-60, 40], dtick=10)
    return fig
