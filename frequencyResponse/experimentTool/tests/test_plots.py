...
from frequencyResponse.experimentTool.tools.plot_mpl import plot_bode_mpl
...
def test_plot_mpl_outputs_png(repo_root: Path):
    ...
    prefix = repo_root / "frequencyResponse" / "experimentTool" / "out" / "plot_unit"
...
def test_plot_plotly_html_if_available(repo_root: Path):
    ...
    if importlib.util.find_spec("plotly") is None:
        return
    from frequencyResponse.experimentTool.tools.plot_plotly import plot_bode_plotly
    spec = ogata_7_25()
    G = build_rational_tf(spec)
    w = np.logspace(-1, 2, 32)
    bode = bode_arrays(G, w, spec.delay, delay_method="frd")
    prefix = repo_root / "frequencyResponse" / "experimentTool" / "out" / "plotly_unit"
