...
def test_cli_lists_and_errors(tmp_path: Path):
    ...
    # list methods
    p1 = subprocess.run([sys.executable, "-m", "pid_controllers.tuningTool.cli", "--file", str(json_path), "--list", "methods"],
                        cwd=REPO_ROOT, capture_output=True, text=True, env=env)
    assert p1.returncode == 0 and "ZN_step" in p1.stdout

    # missing required inputs
    p2 = subprocess.run([sys.executable, "-m", "pid_controllers.tuningTool.cli", "--file", str(json_path),
...
def test_cli_export_json_csv(tmp_path: Path):
    ...
    # export files
    p = subprocess.run([sys.executable, "-m", "pid_controllers.tuningTool.cli", "--file", str(json_path),
