...
```bash
runroot python -m stateSpaceAnalysis.stateSolnTool.cli [OPTIONS]
```
...
runroot mkdir -p stateSpaceAnalysis/stateSolnTool/out
# -----------------------------------------------------------------------------
```
...
## 0) Quick help
```bash
runroot python -m stateSpaceAnalysis.stateSolnTool.cli --help
```
...
```bash
runroot python -m stateSpaceAnalysis.stateSolnTool.cli \
  --example ogata_9_1 \
...
```bash
runroot python -m stateSpaceAnalysis.stateSolnTool.cli --example ogata_9_1 --u 1 --verify
```

### B) Failure mode (missing `--den`): should print an error & nonzero exit
```bash
runroot python -m stateSpaceAnalysis.stateSolnTool.cli --num "1,2,3"
```
