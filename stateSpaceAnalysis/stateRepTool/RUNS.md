...
Package entrypoint:
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli [OPTIONS]
```
...
runroot mkdir -p stateSpaceAnalysis/stateRepTool/out
# -----------------------------------------------------------------------------
```
...
## 0) Help / sanity
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --help
```
...
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --example ogata_9_1 --canonical all
```

### 1.a) Explicit rational expression
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical all
```

### 1.b) CSV coefficients (descending powers)
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --num "1, 3" --den "1, 3, 2" --canonical all
```
...
Controllable:
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical controllable
```
Observable:
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical observable
```
Diagonal (modal): uses partial fractions for simple poles; falls back to eigen-diagonalization when possible.
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical diagonal
```
Jordan (forced even when distinct poles):
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical jordan
```
...
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "1/(s+1)^2" --canonical all
```
...
All forms (8 digits):
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --example ogata_9_1 --canonical all --numeric --digits 8
```
Controllable only, numeric:
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical controllable --numeric
```
CSV + all + numeric (6 digits):
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --num "1, 3" --den "1, 3, 2" --canonical all --numeric --digits 6
```
...
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --example ogata_9_1 --canonical all --export-json ogata9_1_all.json
```
Export to a nested file in out dir:
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --tf "(s+3)/(s^2+3*s+2)" --canonical all --export-json runs/explicit_all.json
```
Export using CSV inputs:
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --num "1, 3" --den "1, 3, 2" --canonical all --export-json csv_all.json
```
...
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli --example ogata_9_1 --canonical all --no-verify
```
...
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli   --tf "(s+3)/(s^2+3*s+2)"   --canonical all --numeric --digits 8   --export-json examples/ogata_9_1_all_numeric.json
```
...
```bash
runroot python -m stateSpaceAnalysis.stateRepTool.cli   --tf "1/(s+1)^2"   --canonical all --no-verify   --export-json examples/repeated_pole_all.json
```
