# stateSpaceAnalysis/stateTransTool/tests/test_cli_smoke.py
...
def run_cmd(args: list[str]) -> tuple[int, str]:
    ...
    repo_root = Path(__file__).resolve().parents[3]  # tests → stateTransTool → stateSpaceAnalysis → modernControl
    p = subprocess.Popen(
        [sys.executable, "-m", "stateSpaceAnalysis.stateTransTool.cli", *args],
        cwd=repo_root,
