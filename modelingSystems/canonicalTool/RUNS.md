...
runroot mkdir -p modelingSystems/canonicalTool/out
# -----------------------------------------------------------------------------
```
...
## 1) Quick help / sanity
```bash
runroot python -m modelingSystems.canonicalTool.cli --help
```

```bash
# Also available when running from inside the package (import shim enabled)
runroot python modelingSystems/canonicalTool/cli.py --help
```
...
runroot python -m modelingSystems.canonicalTool.cli   --num "2,3" --den "1,1,10" --no-plots --no-show
```

```bash
# Another plant, shorter horizon
runroot python -m modelingSystems.canonicalTool.cli   --num "1" --den "1,2,5" --tfinal 5 --no-plots --no-show
```
...
runroot python -m modelingSystems.canonicalTool.cli   --num "2,3" --den "1,1,10" --symbolic --no-plots --no-show
```
...
runroot python -m modelingSystems.canonicalTool.cli   --num "2,3" --den "1,1,10"
```

```bash
# Headless PNG render (no GUI); useful for CI
runroot python -m modelingSystems.canonicalTool.cli   --num "2,3" --den "1,1,10" --save-png "modelingSystems/canonicalTool/out/ccf_ocf_modal.png"   --no-show
```
...
runroot bash -lc 'cd modelingSystems/canonicalTool && python cli.py --num "2,3" --den "1,1,10" --no-plots --no-show'
```
...
runroot python -m modelingSystems.canonicalTool.cli sphinx-skel modelingSystems/canonicalTool/docs
```

```bash
# Build HTML docs (requires sphinx + furo)
runroot python -m pip install sphinx furo
runroot bash -lc 'cd modelingSystems/canonicalTool/docs && make html'
```
...
runroot python -m modelingSystems.canonicalTool.tools.class_diagram
```

```bash
# Manual Graphviz render (if you prefer explicit control)
runroot dot -Tpng modelingSystems/canonicalTool/out/canonicalTool_class_diagram.dot   -o modelingSystems/canonicalTool/out/canonicalTool_class_diagram.png
```
...
runroot pytest modelingSystems/canonicalTool/tests -q
```
