...
runroot mkdir -p modelingSystems/converterTool/out
# -----------------------------------------------------------------------------
```
...
## 1) Quick help / sanity
```bash
runroot python -m modelingSystems.converterTool.cli --help
```
...
```bash
runroot python -m modelingSystems.converterTool.cli \
...
runroot python -m modelingSystems.converterTool.cli \
  --num "1,0" --den "1,14,56,160" --sympy \
  --save-json modelingSystems/converterTool/out/tf2ss_summary.json
```
...
runroot python -m modelingSystems.converterTool.tools.samples
```

```bash
# Emit a Graphviz DOT file (safe on macOS 13.7.x without 'dot')
runroot python -m modelingSystems.converterTool.tools.class_diagram
# -> writes: modelingSystems/converterTool/out/converterTool_class_diagram.dot
# You can render it later on a machine with Graphviz:
# dot -Tpng modelingSystems/converterTool/out/converterTool_class_diagram.dot -o modelingSystems/converterTool/out/converterTool_class_diagram.png
```
...
```bash
runroot python -m modelingSystems.converterTool.cli sphinx-skel docs --force
