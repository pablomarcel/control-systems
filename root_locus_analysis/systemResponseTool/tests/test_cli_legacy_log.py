# root_locus_analysis/systemResponseTool/tests/test_cli_legacy_log.py
...
def run_cmd(args):
    ...
    # (i.e., a directory that contains the 'root_locus_analysis' package folder).
    here = Path(__file__).resolve()
    cur = here.parent
    project_root = None
    for _ in range(10):  # guard to avoid infinite loops
        if (cur / "root_locus_analysis").is_dir():
            project_root = cur
            break
        cur = cur.parent
    ...
    cmd = [python, "-m", "root_locus_analysis.systemResponseTool.cli", "run", *args]
...
def test_cli_accepts_legacy_log_flag():
    code, out = run_cmd([
        ...
        "--tfinal", "0.2",
        "--dt", "0.05",
        "--out-prefix", "root_locus_analysis/systemResponseTool/out/cli_legacy",
