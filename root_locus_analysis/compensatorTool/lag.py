...
from .utils import LOG, error_constants, polyval, tf_arrays
...
def lag_angle_contribution(sstar: complex, z: float, p: float) -> float:
    """Return the absolute lag-network angle contribution in radians."""
    return abs(_wrap_pi(_angle(sstar + z) - _angle(sstar + p)))


def choose_lag_zp_by_angle(
    sstar: complex,
    beta: float,
    ...
    The search uses z equal to 1/T and p equal to 1/(beta*T). Larger T
    values are preferred because they place the lag network closer to the
    origin and reduce the effect on the desired dominant pole location.
    ...
    if best is None:
        # Fall back to largest T and report the resulting angle.
        T = T_grid[-1]
        z = 1.0 / T
        p = 1.0 / (beta * T)
...
class LagCompensatorService:
    ...
    The service improves the selected steady-state error constant using a
    requested target, an improvement factor, or a chosen beta value. It then
    sizes the compensator gain from the loop-magnitude condition at the
    ...
    def design(
        ...
        if beta is None:
            if factor is not None:
                ...
                if not np.isfinite(base_const) or base_const <= 0:
                    raise ValueError(f"Cannot size beta from target: current {key} is {base_const}.")
                beta = max(1.01, float(target) / float(base_const))
            else:
                beta = 10.0

        if beta <= 1.0:
            raise ValueError("Require beta > 1 for a lag network.")

        LOG.info("beta (z/p) = %.6g", beta)
        ...
        if z_user is not None and p_user is not None:
            ...
            LOG.info("Using user z/p: z=%.6g, p=%.6g (angle %.3g deg)", z, p, angle_deg)
        else:
            if T_user is not None:
                ...
                p = 1.0 / (beta * T)
                angle_deg = math.degrees(lag_angle_contribution(sstar, z, p))
                LOG.info("Using user T=%.6g -> z=%.6g, p=%.6g (angle %.3g deg)", T, z, p, angle_deg)
            else:
                z, p, angle_deg = choose_lag_zp_by_angle(sstar, beta, thetamax)
                LOG.info(
                    "Auto-placed z/p: z=%.6g, p=%.6g with angle=%.3g deg (<= %.3g deg)",
                    z,
                    p,
                    angle_deg,
                    thetamax,
        ...
        # Magnitude condition at the desired pole.
        nG, dG = tf_arrays(G)
        Lmag = abs(polyval(nG, sstar) / polyval(dG, sstar))
        Kc = 1.0 / (Lmag * abs((sstar + z) / (sstar + p)))
        LOG.info("Kc from loop-magnitude condition at sstar: %.6g", Kc)
        ...
        return LagDesignResult(
            ...
            beta=beta,
            angle_deg=angle_deg,
            Gc=ct.minreal(Gc, verbose=False),
            L=L,
            before=consts0,
            after=error_constants(L),
        )
...
class LagCompensatorApp:
    ...
    def run(self, **kwargs) -> LagDesignResult:
        """
        Run a lag-compensator design from caller-provided keyword arguments.

        The caller provides a resolved pole specification, plant coefficients,
        and lag-specific options. The method prints a compact text report and
        returns the computed design result.
        """
        ...
        print(f"beta (z/p):              {res.beta:.6g}")
        print(f"Zero z  (at s=-z):       {res.z:.6g}")
        print(f"Pole p  (at s=-p):       {res.p:.6g}")
        print(f"Angle at sstar:          {res.angle_deg:.3g} deg")
        print(f"Kc from magnitude rule:  {res.Kc:.6g}")
