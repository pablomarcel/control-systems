...
## Run (from repo root)
```bash
python -m modeling_control_systems.canonicalTool.cli --num "1" --den "1,2,5" --tfinal 5 --no-show
```

## Generate Sphinx skeleton
```bash
python -m modeling_control_systems.canonicalTool.cli sphinx-skel docs
```

## Class diagram (.dot; PNG if Graphviz `dot` is available)
```bash
python -m modeling_control_systems.canonicalTool.tools.class_diagram
```
