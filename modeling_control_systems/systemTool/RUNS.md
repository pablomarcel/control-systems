...
runroot mkdir -p modeling_control_systems/systemTool/out
# -----------------------------------------------------------------------------
```
...
runroot python -m modeling_control_systems.systemTool.cli --help

# Direct script (equivalent)
runroot python modeling_control_systems/systemTool/cli.py --help
```

### 1.1 Mass–Spring–Damper step
```bash
# Basic (defaults: m=1, b=1, k=10, u0=1, tfinal=10, dt=1e-3)
runroot python -m modeling_control_systems.systemTool.cli msd-step --no-save
```

```bash
# With parameters and PNG output
runroot python -m modeling_control_systems.systemTool.cli msd-step \
  --m 1 --b 1 --k 10 --u0 1 --tfinal 5 --dt 0.001
# -> writes modeling_control_systems/systemTool/out/msd_y.png and msd_states.png
```

### 1.2 Compute G(s) from (A,B,C,D)
```bash
# Use default MSD model (C = [1,0]) and print transfer function(s)
runroot python -m modeling_control_systems.systemTool.cli tf-from-ss
```
...
runroot python -m modeling_control_systems.systemTool.cli tf-from-ss --m 1 --b 1 --k 10
```

### 1.3 MIMO demo (2×2 example)
```bash
runroot python -m modeling_control_systems.systemTool.cli mimo-demo
...
runroot python -m modeling_control_systems.systemTool.cli ode-no-deriv \
  --a "[1, 10]" --b0 1 --tfinal 8 --dt 0.001
# -> writes modeling_control_systems/systemTool/out/ode_no_deriv_step.png
```

```bash
# Convenience MSD mapping: y'' + (b/m) y' + (k/m) y = (1/m) u
runroot python -m modeling_control_systems.systemTool.cli ode-no-deriv --msd --m 1 --b 1 --k 10 --tfinal 6 --dt 0.001
```

### 1.5 ODE with derivatives of u (Ogata §2-5 β-construction)
```bash
# Defaults show a D=b0 jump for a unit step
runroot python -m modeling_control_systems.systemTool.cli ode-with-deriv --tfinal 8 --dt 0.001
```

```bash
# Explicit numerator/denominator: y'' + a1 y' + a2 y = b0 u'' + b1 u' + b2 u
runroot python -m modeling_control_systems.systemTool.cli ode-with-deriv \
  --a "[1,10]" \
  --b "[0.5, 1.0, 0.0]" \
  --tfinal 8 --dt 0.001
# -> writes modeling_control_systems/systemTool/out/ode_with_deriv_step.png
```

### 1.6 Kelvin–Voigt vs Maxwell comparison
```bash
# Overlay y(t) and F(t) for both models
runroot python -m modeling_control_systems.systemTool.cli kv-vs-maxwell --u0 1 --tfinal 10 --dt 0.001
# -> writes modeling_control_systems/systemTool/out/kv_vs_maxwell_y.png
# -> writes modeling_control_systems/systemTool/out/kv_vs_maxwell_force.png
...
runroot python modeling_control_systems/systemTool/tools/class_diagram.py --out modeling_control_systems/systemTool/out/systemTool_class_diagram.mmd
```
...
runroot python -m modeling_control_systems.systemTool.cli sphinx-skel docs --force
