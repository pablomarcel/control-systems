...
```bash
cd transientAnalysis/routhTool
```
...
# -> writes transientAnalysis/routhTool/out/demo.json
```
...
```bash
python -m transientAnalysis.routhTool.cli --coeffs "1, 5, 6, 2" --verify
python -m transientAnalysis.routhTool.cli --coeffs "1, 5, 6, K" --symbol K --solve-for K
python -m transientAnalysis.routhTool.cli --coeffs "1, 2, 3, 4, K" --symbol K --solve-for K --hurwitz
python -m transientAnalysis.routhTool.cli --coeffs "1, 0, 2, 0, 1" --verify
python -m transientAnalysis.routhTool.cli --coeffs "1, 5, 6, 2" --export demo
```
