...
```bash
pytest state_space_design/minOrdTfTool/tests -q
```

- **With coverage** (only if `.coveragerc` exists at the path below):  
```bash
pytest state_space_design/minOrdTfTool/tests \
  --override-ini addopts= \
  --cov \
  --cov-config=state_space_design/minOrdTfTool/.coveragerc \
...
```bash
plantuml state_space_design/minOrdTfTool/tools/class_diagram.puml
```
