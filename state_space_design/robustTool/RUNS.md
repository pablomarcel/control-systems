...
- From repo root:
  ```bash
  python -m state_space_design.robustTool.cli --help
  ```
...
```bash
python -m state_space_design.robustTool.cli \
  --num="10 20" --den="1 10 24 0" \
  --pid="2,5,0.1,20" \
  --Wm_num="0.2 1" --Wm_den="0.02 1" \
  --Ws_num="0.5 0" --Ws_den="1 0.05 0" \
...
```bash
pytest state_space_design/robustTool/tests \
  --override-ini addopts= \
  --cov \
  --cov-config=state_space_design/robustTool/.coveragerc \
