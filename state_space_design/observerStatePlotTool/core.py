...
class ObserverStateProcessor:
    def reconstruct_u_y_if_missing(self, payload: Dict[str, Any], t: np.ndarray,
        ...
        """
        Robustly reconstruct u = -K(x-e) and y = Cx if they're missing.
        Tolerates JSONs with "simulation": null by treating it like {}.
        """
        simblk = payload.get("simulation", None)
        if not isinstance(simblk, dict):
            simblk = {}
        u = safe_series("simulation.u", simblk.get("u", None))
        y = safe_series("simulation.y", simblk.get("y", None))
    ...
    def simulate_if_missing(self, payload: Dict[str, Any], T: np.ndarray,
        ...
        """
        Simulate ż = A_aug z with z=[x; e] using fixed-step discretization via expm.
        Reconstruct u,y when possible from K and C.
        """
    ...
    def load_or_simulate(self, payload: Dict[str, Any], simulate: bool,
        ...
        """
        Use a present simulation (if a dict) unless --simulate_if_missing requested,
        otherwise simulate from A_augmented.
        """
        sim = payload.get("simulation", None)

        # Only use "existing simulation" path if it's a dict (not None) and user didn't request simulate
        if isinstance(sim, dict) and not simulate:
    ...
    def choose_series(self, want: List[str], X, E, y, u) -> tuple[list[str], list[np.ndarray]]:
        """
        Build (labels, series) in the order requested by 'want'.
        Supports MIMO y (multiple rows) and ensures 2D → 1D flattening per series.
        """
