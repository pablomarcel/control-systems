...
def parse_matrix(s: str) -> np.ndarray:
    """
    Parse a matrix from strings like:
      "0 1; -2 -3" or "0,1; -2,-3" or with newlines.
...
def pretty_poly(coeffs: np.ndarray, var: str = "s") -> str:
    ...
    for i, a in enumerate(coeffs):
        ...
        elif p == 1: terms.append(f"{a:.6g}{var}")
        else:        terms.append(f"{a:.6g}{var}^{p}")
...
def pole_multiplicities(poles: np.ndarray, tol: float = 0.0) -> Dict[float, int]:
    ...
    if p_sorted.size == 0: return groups
    ref = p_sorted[0]; count = 1
    for v in p_sorted[1:]:
        if abs(v - ref) <= tol: count += 1
        else:
