...
runroot mkdir -p control_systems/canonicalTool/out
# -----------------------------------------------------------------------------
```
...
## 1) Quick help / sanity
```bash
runroot python -m control_systems.canonicalTool.cli --help
```

```bash
# Also available when running from inside the package (import shim enabled)
runroot python control_systems/canonicalTool/cli.py --help
```
...
runroot python -m control_systems.canonicalTool.cli   --num "2,3" --den "1,1,10" --no-plots --no-show
```

```bash
# Another plant, shorter horizon
runroot python -m control_systems.canonicalTool.cli   --num "1" --den "1,2,5" --tfinal 5 --no-plots --no-show
```
...
runroot python -m control_systems.canonicalTool.cli   --num "2,3" --den "1,1,10" --symbolic --no-plots --no-show
```
...
runroot python -m control_systems.canonicalTool.cli   --num "2,3" --den "1,1,10"
```

```bash
# Headless PNG render (no GUI); useful for CI
runroot python -m control_systems.canonicalTool.cli   --num "2,3" --den "1,1,10" --save-png "modelingSystems/canonicalTool/out/ccf_ocf_modal.png"   --no-show
```
...
runroot bash -lc 'cd control_systems/canonicalTool && python cli.py --num "2,3" --den "1,1,10" --no-plots --no-show'
```
...
runroot python -m control_systems.canonicalTool.cli sphinx-skel control_systems/canonicalTool/docs
```

```bash
# Build HTML docs (requires sphinx + furo)
runroot python -m pip install sphinx furo
runroot bash -lc 'cd control_systems/canonicalTool/docs && make html'
```
...
runroot python -m control_systems.canonicalTool.tools.class_diagram
```

```bash
# Manual Graphviz render (if you prefer explicit control)
runroot dot -Tpng control_systems/canonicalTool/out/canonicalTool_class_diagram.dot   -o control_systems/canonicalTool/out/canonicalTool_class_diagram.png
```
...
runroot pytest control_systems/canonicalTool/tests -q
```
