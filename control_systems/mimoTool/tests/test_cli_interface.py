...
def _cli_file() -> pathlib.Path:
    mod = importlib.import_module("control_systems.mimoTool.cli")
...
def test_cli_sphinx_skel(tmp_path):
    docs = tmp_path / "docs"
    res = subprocess.run([sys.executable, "-m", "control_systems.mimoTool.cli", "sphinx-skel", str(docs)], capture_output=True, text=True, check=True)
