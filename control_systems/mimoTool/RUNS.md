...
runroot mkdir -p control_systems/mimoTool/out
# -----------------------------------------------------------------------------
```
...
runroot python -m control_systems.mimoTool.cli --plant two_tank --no-show --no-steps --no-sigma \
  --save-json "modelingSystems/mimoTool/out/{plant}_summary.json"
```

```bash
# Thermal plant only
runroot python -m control_systems.mimoTool.cli --plant two_zone_thermal --no-show --no-steps --no-sigma \
  --save-json "modelingSystems/mimoTool/out/{plant}_summary.json"
```
...
runroot python -m control_systems.mimoTool.cli \
  --plant two_tank \
  --no-show \
...
runroot python -m control_systems.mimoTool.cli sphinx-skel control_systems/mimoTool/docs
```

```bash
# Build HTML docs to _build/html (requires sphinx, furo)
runroot python -m sphinx -b html control_systems/mimoTool/docs control_systems/mimoTool/docs/_build/html
```
...
runroot pytest control_systems/mimoTool/tests \
  --override-ini addopts= \
  --cov \
  --cov-config=control_systems/mimoTool/.coveragerc \
