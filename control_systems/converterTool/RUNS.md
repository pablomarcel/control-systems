...
runroot mkdir -p control_systems/converterTool/out
# -----------------------------------------------------------------------------
```
...
## 1) Quick help / sanity
```bash
runroot python -m control_systems.converterTool.cli --help
```
...
```bash
runroot python -m control_systems.converterTool.cli \
...
runroot python -m control_systems.converterTool.cli \
  --num "1,0" --den "1,14,56,160" --sympy \
  --save-json control_systems/converterTool/out/tf2ss_summary.json
```
...
runroot python -m control_systems.converterTool.tools.samples
```

```bash
# Emit a Graphviz DOT file (safe on macOS 13.7.x without 'dot')
runroot python -m control_systems.converterTool.tools.class_diagram
# -> writes: control_systems/converterTool/out/converterTool_class_diagram.dot
# You can render it later on a machine with Graphviz:
# dot -Tpng control_systems/converterTool/out/converterTool_class_diagram.dot -o control_systems/converterTool/out/converterTool_class_diagram.png
```
...
```bash
runroot python -m control_systems.converterTool.cli sphinx-skel docs --force
